...
module.exports = {
    ...
    async execute(message, args, a, b, c, profileData) {
        ...
        if (afkreason.length > 1000) {
            return message.reply("Keep afk reason under 1000 characters.")
        }
        ...
        message.member.setNickname(`[AFK] ${message.member.nickname || message.author.username}`).catch(e => {
            message.reply("Couldn't update your nickname.").then(m => {
                setTimeout(() => {
                    m.delete()
                }, 5000)
            })
